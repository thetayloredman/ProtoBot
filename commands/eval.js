...
// Main
async function run(client, message, args, log) {
    /**
...
${code}`);
    ...
    const embed = new discord_js_1.default.MessageEmbed().setFooter(`Eval command executed by ${message.author.username}`).setTimestamp();
    let msg;
    let response;
    let e = false;
    try {
