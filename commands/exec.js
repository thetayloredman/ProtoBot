...
function run(client, message, args, log) {
    ...
    if (message.author.id !== client.config.ownerID) {
        log('w', `User ${message.author.tag} tried to use exec! Code:
${code}`);
        message.reply("You don't have permission to do that!");
        return;
    }
    const embed = new discord_js_1.default.MessageEmbed().setFooter(`Exec command executed by ${message.author.username}`).setTimestamp();
    let e = false;
    child_process_1.exec(code, (error, stdout, stderr) => {
        if (error || stderr) {
            e = true;
        }
        if (stderr) {
        ...
        const parsed = [
            (error !== null && error !== void 0 ? error : { toString: () => '' }).toString(),
            stderr,
            stdout
