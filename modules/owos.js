...
function run(client, message, log) {
    ...
    if (!cooldowns || cooldowns + client.config.cooldowns.owos - Date.now() < 1) {
        ...
        if (message.content.toLowerCase().includes('owo') && !hasPrefix) {
            ...
            log('i', `${chalk_1.default.green('[')}${chalk_1.default.green.bold('OwOHandler')}${chalk_1.default.green(']')} ${chalk_1.default.red('[')}${chalk_1.default.red.bold('+')}${chalk_1.default.red(']')} Added owo!`);
        }
    }
    else {
        log('i', `${chalk_1.default.green('[')}${chalk_1.default.green.bold('OwOHandler')}${chalk_1.default.green(']')} User still on cooldown! ${chalk_1.default.red(cooldowns + client.config.cooldowns.owos - Date.now())} ms remaining!`);
    }
}
exports.run = run;
// Config
